...
class ArticleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('title','description', 'content', 'tags', 'auto_tag',  'status')}), #delete 'markup', 
        ('Metadata', {
            'fields': ('keywords', ), #'description',), Bone switch it to content before
            'classes': ('collapse',)
        }),
        ('Relationships', {
        ...
        ('Advanced', {
            #'fields': ('slug', 'is_active', 'login_required', 'sites'),  Typing a nume for slug is disgusting when .js can't get a slug, specially in the condition Chinese can't be recognized and changed into null. But it's very convenient tansform title to slug automatically. Changed by Spark
            'fields': ('is_active', 'login_required', 'sites'),
            'classes': ('collapse',)
        }),
    )

    filter_horizontal = ('tags', 'followup_for', 'related_articles')
    #prepopulated_fields = {'slug': ('title',)} #   Deleted by Spark
